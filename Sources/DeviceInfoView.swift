...
public extension Image {
    init(symbolName: String) {
        ...
        if UIImage(systemName: symbolName) != nil {
            ...
            self.init(symbolName, bundle: Bundle(for: ActualHardwareDevice.self))
        }
...
public extension String {
    func safeSymbolName(fallback: String = "questionmark.square.fill") -> String {
        ...
        if UIImage(systemName: self) == nil {
            // check for asset
            if UIImage(named: self, in: Bundle(for: ActualHardwareDevice.self), compatibleWith: nil) == nil {
                return fallback
            }
        }
#elseif canImport(AppKit)
        if NSImage(systemSymbolName: self, accessibilityDescription: nil) != nil {
            // check for asset
            if NSImage(named: self, in: Bundle(for: ActualHardwareDevice.self), compatibleWith: nil) == nil {
                return fallback
            }
        }
#endif
